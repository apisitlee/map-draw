...
export const SearchPanel: React.FC = () => {
    ...
    windowAMap.plugin('AMap.LineSearch', () => {
      ...
      lineSearch.search(lineSearchName, (status: string, result: any) => {
        if (status === 'complete' && result.lineInfo && result.lineInfo.length > 0) {
          ...
          showAlert(`检索线路 "${lineSearchName}" 详细数据失败`);
        }
      });
    });
  };
  ...
  return (
    <>
      <div className="fixed top-0 left-0 bottom-0 right-0 bg-black/20 z-60 backdrop-blur-xs" onClick={() => setIsSearchPanelOpen(false)}></div>
      <div className="absolute top-10 left-1/2 -translate-x-1/2 w-[480px] max-h-[calc(100vh-90px)] bg-white/88 backdrop-blur-xl rounded-2xl shadow-xl border border-white/70 z-61 flex flex-col transition-all overflow-hidden">
        ...
        {(keyword || results.length > 0) && (
          <div className="p-3 shadow-md">
            <div className="bg-black/5 p-0.5 rounded-lg flex gap-0.5">
              {[
                { id: 'all', label: '全部' },
                { id: 'point', label: '地点', icon: <MapPin className="w-3.5 h-3.5" /> },
                { id: 'line', label: '线路', icon: <Route className="w-3.5 h-3.5" /> },
                { id: 'station', label: '站点', icon: <Bus className="w-3.5 h-3.5" /> },
              ].map((cat) => (
                <div
                  key={cat.id}
                  className={`flex-1 flex items-center justify-center text-center py-1 text-[11px] cursor-pointer rounded-md font-medium transition-all select-none ${currentCategory === cat.id
        ...
        <div className="flex-1 bg-[#f7f8f9] flex flex-col overflow-y-auto py-3">
          {/* Results list */}
          {isSearching ? (
            <div className="text-[#8e8e93] text-center text-xs px-3 py-4 flex items-center justify-center gap-2">
              <Loader2 className="w-4 h-4 animate-spin text-[#007AFF]" />
              正在检索...
            </div>
          ) : results.length === 0 ? (
            keyword && (
              <div className="text-[#8e8e93] text-center text-xs px-3 py-4">未查找到匹配数据</div>
            )
          ) : (
              ...
              {results.map((item) => (
                ...
                >
                  <div className="flex items-center justify-between w-full">
                    <div
                      className={`w-6 h-6 rounded-md flex items-center justify-center text-xs shrink-0 ${item.type === 'line'
        ...
        {/* Pagination Bar */}
        {results.length > 0 && (
          <div className="flex items-center justify-between shadow-md py-2 px-3 border-t border-black/5 text-xs text-[#8e8e93]">
            <button
